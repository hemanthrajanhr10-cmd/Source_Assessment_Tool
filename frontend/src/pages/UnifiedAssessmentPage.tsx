...
const activeBorderCls = 'border-[rgba(22,163,74,0.45)]'
...
const inputCls = [
  ...
  'focus:outline-none focus:border-[rgba(22,163,74,0.6)] focus:ring-1 focus:ring-[rgba(22,163,74,0.15)]',
  'transition-colors',
].join(' ')
...
function StepHeader({
  ...
  return (
    <div className={`flex items-center gap-3 px-5 py-4 border-b ${
      done ? 'border-slate-200 bg-white' : active ? 'border-[rgba(22,163,74,0.10)] bg-earth-50' : 'border-slate-200 bg-white'
    }`}>
      <span className={`flex items-center justify-center h-6 w-6 rounded-full text-xs font-bold shrink-0 ${
          ...
          : active
          ? 'bg-[rgba(22,163,74,0.10)] text-earth-700'
      ...
      {count && (
        <span className="text-xs font-medium text-earth-700">{count}</span>
      )}
    </div>
  )
}
    ...
    <label className="flex items-start gap-3 cursor-pointer">
      <button
        ...
        className={`relative mt-0.5 inline-flex h-5 w-9 shrink-0 items-center rounded-full border-2 border-transparent
          transition-colors focus:outline-none focus-visible:ring-2 focus-visible:ring-[rgba(22,163,74,0.30)]
          ${checked ? 'bg-[#16A34A]' : 'bg-slate-200'}`}
      >
              ...
              <>
                    ...
                    return (
                      <button
                        ...
                        className={`flex flex-col items-start gap-2 p-4 rounded-xl border text-left transition-all duration-150 ${
                          selected
                            ? 'border-[rgba(22,163,74,0.40)] bg-[rgba(22,163,74,0.06)]'
                            : 'border-slate-200 bg-slate-50 hover:border-slate-300'
                        }`}
                      >
                        <div className={`h-8 w-8 rounded-lg flex items-center justify-center ${
                          selected ? 'bg-[rgba(22,163,74,0.15)]' : 'bg-slate-200'
                        }`}>
                          <Icon className={`h-4 w-4 ${selected ? 'text-earth-700' : 'text-slate-500'}`} />
                        </div>
                        <div>
                        ...
                        {selected && (
                          <div className="w-full flex justify-end">
                            <CheckCircle2 className="h-3.5 w-3.5 text-earth-700" />
                          </div>
                        )}
                      </button>
                    )
                ...
                <button
                  ...
                  className="flex items-center gap-2 px-4 py-2.5 rounded-xl text-sm font-semibold
                             bg-[#166534] text-white hover:bg-[#16A34A] transition-colors
                             focus:outline-none focus:ring-2 focus:ring-[rgba(22,163,74,0.30)]"
                  style={{ boxShadow: '0 0 20px rgba(22,163,74,0.10)' }}
                >
            ...
            <div className="p-5 space-y-4">
              ...
              <button
                ...
                className="w-full flex items-center justify-center gap-2 px-4 py-3 rounded-xl text-sm font-semibold
                           bg-[#166534] text-white hover:bg-[#16A34A] disabled:opacity-50
                           transition-colors focus:outline-none focus:ring-2 focus:ring-[rgba(22,163,74,0.30)]"
                style={{ boxShadow: phase !== 'fabric-submitting' ? '0 0 24px rgba(22,163,74,0.12)' : undefined }}
              >
  ...
  return (
    <div className="space-y-4">
      ...
      <button
        ...
        className="w-full flex items-center justify-center gap-2 px-4 py-3 rounded-xl text-sm font-semibold
                   bg-[#166534] text-white hover:bg-[#16A34A] disabled:opacity-40
                   transition-colors focus:outline-none focus:ring-2 focus:ring-[rgba(22,163,74,0.30)]"
        style={{ boxShadow: (!submitting && totalDbs > 0) ? '0 0 20px rgba(22,163,74,0.10)' : undefined }}
      >
        {submitting
      ...
      {open && (
        <div className="absolute top-full mt-1.5 left-0 z-50 w-72 rounded-xl border border-slate-200 bg-white overflow-hidden"
          ...
          <div className="flex items-center gap-2 px-3 py-2 border-b border-slate-200">
            <Share2 className="h-3.5 w-3.5 text-earth-700" />
          ...
          {!loading && !error && connections.length === 0 && (
            <div className="px-3 py-4 text-xs text-slate-500 text-center">
              No saved connections. <a href="/hybrid-connection" className="text-earth-700 underline underline-offset-2">Create one</a> first.
            </div>
          )}
          {!loading && connections.map(hc => (
            ...
            >
              <div className="h-7 w-7 rounded-lg bg-[rgba(22,163,74,0.08)] border border-[rgba(22,163,74,0.10)] flex items-center justify-center shrink-0 mt-0.5">
                <Share2 className="h-3.5 w-3.5 text-earth-700" />
              </div>
    ...
    <div className="rounded-xl border border-slate-200 bg-slate-50 overflow-hidden">
      ...
      >
        <div className="h-7 w-7 rounded-lg flex items-center justify-center shrink-0"
          style={{ background: 'rgba(22,163,74,0.08)', border: '1px solid rgba(22,163,74,0.10)' }}>
        ...
        {srv.selected_dbs.length > 0 && (
          <span className="text-xs font-mono text-earth-700">{srv.selected_dbs.length} DB{srv.selected_dbs.length!==1?'s':''}</span>
        )}
        {srv.connectivity && (
            ...
            <div className="space-y-1.5">
              <div className="flex items-center justify-between">
                ...
                <button
                  ...
                  className="text-[10px] font-semibold text-earth-700 hover:text-[#16A34A] uppercase tracking-widest transition-colors"
                >
                  ...
                  return (
                    <label key={db.name}
                      className={`flex items-center gap-2.5 px-3 py-2 rounded-lg cursor-pointer transition-colors ${
                        selected ? 'bg-[rgba(22,163,74,0.06)] hover:bg-[rgba(22,163,74,0.09)]' : 'hover:hover:bg-slate-50'
                      }`}>
                      <input
                        type="checkbox"
                        checked={selected}
                        ...
                        className="rounded border-slate-200 accent-[#16A34A]"
                      />
  ...
  if (!userCode) {
    return (
      <div className="flex items-center gap-3 text-sm text-slate-500">
        <Loader2 className="h-4 w-4 animate-spin text-earth-700" />
  ...
  return (
    <div className="space-y-4">
      ...
      <a
        ...
        className="flex items-center gap-2 text-sm font-medium text-earth-700 hover:text-[#16A34A] hover:underline transition-colors"
      >
        <ExternalLink className="h-4 w-4 shrink-0" />
        {verificationUrl}
      </a>
      <div className="flex items-stretch gap-3">
        <div className="flex-1 rounded-xl border-2 border-[rgba(22,163,74,0.20)] bg-[rgba(22,163,74,0.06)] px-5 py-3 text-center">
      ...
      <div className="flex items-center gap-2 text-xs text-slate-500">
        <Loader2 className="h-3.5 w-3.5 animate-spin text-earth-700" />
    ...
    <div className="space-y-4">
      <div className="flex items-center gap-3">
        ...
        <button
          ...
          className="shrink-0 text-xs text-earth-700 hover:text-[#16A34A] font-semibold transition-colors"
        >
          ...
          {filtered.map(ws => (
            ...
            >
              <input
                ...
                className="h-4 w-4 rounded border-slate-200 accent-[#16A34A]"
              />
              ...
              {selectedIds.has(ws.id) && <CheckCircle2 className="h-4 w-4 text-earth-700 shrink-0" />}
            </label>
          ))}
          {filtered.length === 0 && (
            ...
            <div key={wi.workspace_id}>
              ...
              {expanded && (
                <div className="divide-y divide-slate-100">
                  {filteredDs.length > 0 && (
                    <div>
                      <div className="flex items-center gap-2 px-5 py-1.5 bg-[rgba(22,163,74,0.03)]">
                        <Database className="h-3 w-3 text-earth-700 shrink-0" />
                        <span className="text-xs font-medium text-slate-400 flex-1">Semantic Models</span>
                        <button onClick={toggleAllDs} className="text-xs text-earth-700 hover:text-[#16A34A] transition-colors">
                      ...
                      {filteredDs.map(ds => (
                        <label key={ds.id} className="flex items-center gap-3 px-6 py-2 hover:bg-[rgba(255,255,255,0.02)] cursor-pointer transition-colors">
                          <input
                            type="checkbox"
                            className="h-3.5 w-3.5 rounded border-slate-200 accent-[#16A34A]"
                  ...
                  {filteredRp.length > 0 && (
                    <div>
                      <div className="flex items-center gap-2 px-5 py-1.5 bg-[rgba(96,165,250,0.03)]">
                        ...
                        <button onClick={toggleAllRp} className="text-xs text-earth-700 hover:text-[#16A34A] transition-colors">
