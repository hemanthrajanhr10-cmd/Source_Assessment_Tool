...
function CopyButton({ text }: { text: string }) {
  ...
  return (
    <button
      ...
      style={{
        background: copied ? 'rgba(5,150,105,0.10)' : 'rgba(22,163,74,0.07)',
        color: copied ? '#047857' : '#16A34A',
        border: `1px solid ${copied ? 'rgba(5,150,105,0.25)' : 'rgba(22,163,74,0.18)'}`,
      }}
...
function MeasureRow({ m }: { m: FabricMeasure }) {
  ...
  return (
    <div className="rounded-xl overflow-hidden transition-all" style={{ border: '1px solid rgba(187,247,208,0.8)', boxShadow: open ? '0 2px 12px rgba(22,163,74,0.08)' : undefined }}>
      <button onClick={() => setOpen(o => !o)}
        className="w-full flex items-center gap-2 px-3 py-2.5 text-left transition-colors"
        style={{ background: open ? 'linear-gradient(135deg, rgba(22,163,74,0.05) 0%, rgba(22,163,74,0.02) 100%)' : 'rgba(248,250,253,0.8)' }}>
      ...
      {open && (
        <div className="border-t p-3 space-y-2.5" style={{ borderColor: 'rgba(187,247,208,0.5)', background: 'rgba(248,250,253,0.6)' }}>
          {m.expression && (
            <div>
              ...
              <pre className="text-xs font-mono rounded-lg px-3 py-2 overflow-x-auto whitespace-pre-wrap text-slate-700 max-h-32 border"
                style={{ background: 'rgba(22,163,74,0.03)', borderColor: 'rgba(22,163,74,0.10)' }}>
                {m.expression}
              </pre>
            </div>
          )}
...
function CalcItemRow({ item, type }: { item: FabricCalculatedColumn | FabricCalculatedTable; type: 'col' | 'table' }) {
  ...
  return (
    <div className="rounded-xl overflow-hidden transition-all" style={{ border: '1px solid rgba(187,247,208,0.8)', boxShadow: open ? '0 2px 12px rgba(22,163,74,0.08)' : undefined }}>
      ...
      {open && hasExpr && (
        <div className="border-t p-3" style={{ borderColor: 'rgba(187,247,208,0.5)', background: 'rgba(248,250,253,0.6)' }}>
          <div className="flex items-center justify-between mb-1.5">
            <p className="text-xs font-semibold text-slate-500 flex items-center gap-1">
              <Code2 className="h-3 w-3" /> DAX Expression
            </p>
    ...
    <div className="border border-slate-200 rounded-xl overflow-hidden">
      ...
      {open && (
        <div className="border-t border-slate-100">
          ...
          {/* Sub-tab nav */}
          <div className="flex px-4 overflow-x-auto" style={{ borderBottom: '1px solid rgba(187,247,208,0.6)', background: 'rgba(248,250,253,0.6)' }}>
            {SUB_TABS.map(t => (
              <button key={t.id}
                ...
                style={{
                  borderBottomColor: subTab === t.id
                    ? (t.id === 'source_feeds' ? '#0F766E' : '#16A34A')
                    : 'transparent',
                  color: subTab === t.id
                    ? (t.id === 'source_feeds' ? '#0F766E' : '#166534')
                    : '#64748B',
                }}>
                ...
                <span className="rounded-full px-1.5 py-0.5 text-xs font-semibold"
                  style={{
                    background: subTab === t.id
                      ? (t.id === 'source_feeds' ? 'rgba(13,148,136,0.12)' : 'rgba(22,163,74,0.10)')
...
function OverviewTab({ workspaces, summary }: { workspaces: FabricWorkspace[]; summary: any }) {
  ...
  const KPI_ITEMS = [
    { label: 'Workspaces',      value: summary.workspace_count,          iconBg: 'linear-gradient(135deg,#166534,#16A34A)',  glow: 'rgba(22,163,74,0.20)',   icon: <Zap className="h-4 w-4 text-white" />,        numColor: '#166534' },
    { label: 'Semantic Models', value: summary.dataset_count,            iconBg: 'linear-gradient(135deg,#15803D,#22C55E)',  glow: 'rgba(22,163,74,0.20)',  icon: <Database className="h-4 w-4 text-white" />,   numColor: '#15803D' },
    { label: 'Reports',         value: summary.report_count,             iconBg: 'linear-gradient(135deg,#059669,#34D399)',  glow: 'rgba(5,150,105,0.20)',  icon: <FileText className="h-4 w-4 text-white" />,   numColor: '#059669' },
    ...
    { label: 'Total Visuals',   value: summary.total_visuals ?? 0,       iconBg: 'linear-gradient(135deg,#4ADE80,#86EFAC)',  glow: 'rgba(74,222,128,0.20)', icon: <Eye className="h-4 w-4 text-white" />,        numColor: '#16A34A' },
    { label: 'Measures',        value: summary.total_measures,           iconBg: 'linear-gradient(135deg,#166534,#4ADE80)',  glow: 'rgba(22,163,74,0.20)',  icon: <Hash className="h-4 w-4 text-white" />,       numColor: '#166534' },
  ...
  return (
    <div className="space-y-6">
        ...
        {KPI_ITEMS.map(({ label, value, iconBg, glow, icon, numColor }) => (
          <div key={label}
            ...
            onMouseLeave={e => {
              ...
              el.style.borderColor = 'rgba(187,247,208,0.8)'
            }}
          >
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-5">
        <div className="rounded-2xl border p-5" style={{ borderColor: 'rgba(187,247,208,0.8)', boxShadow: '0 2px 8px rgba(22,163,74,0.04)' }}>
      ...
      {/* ── Workspace table ───────────────────────────────────────────────────── */}
      <div className="rounded-2xl border overflow-hidden" style={{ borderColor: 'rgba(187,247,208,0.8)', boxShadow: '0 2px 8px rgba(22,163,74,0.04)' }}>
        <div className="px-5 py-3.5 flex items-center gap-3"
          style={{ background: 'linear-gradient(135deg, rgba(22,163,74,0.06) 0%, rgba(22,163,74,0.03) 100%)', borderBottom: '1px solid rgba(187,247,208,0.6)' }}>
          <span className="flex items-center justify-center h-6 w-6 rounded-lg"
            style={{ background: 'linear-gradient(135deg,#166534,#16A34A)', boxShadow: '0 2px 6px rgba(22,163,74,0.25)' }}>
        ...
        <div className="overflow-x-auto">
          <table className="w-full text-xs">
            <thead>
              <tr style={{ background: 'rgba(239,246,255,0.7)', borderBottom: '1px solid rgba(187,247,208,0.6)' }}>
            ...
            <tbody>
              {workspaces.map((ws) => (
                <tr key={ws.id}
                  ...
                  onMouseEnter={e => { (e.currentTarget as HTMLTableRowElement).style.background = 'rgba(22,163,74,0.03)' }}
                ...
                >
                  ...
                  <td className="px-4 py-2.5 text-center font-bold" style={{ color: '#65A30D' }}>{ws.dataset_count}</td>
                  <td className="px-4 py-2.5 text-center font-bold" style={{ color: '#059669' }}>{ws.report_count}</td>
                  <td className="px-4 py-2.5 text-center font-bold text-amber-600">{ws.paginated_report_count}</td>
                  <td className="px-4 py-2.5 text-center font-bold" style={{ color: '#16A34A' }}>{ws.reports.reduce((s, r) => s + (r.visual_count ?? 0), 0)}</td>
          ...
          {workspaces.map((ws, wi) => {
            ...
            return (
              <motion.button
                ...
                onMouseEnter={e => {
                  e.currentTarget.style.border = '1.5px solid rgba(22,163,74,0.42)'
                  e.currentTarget.style.boxShadow = '0 8px 28px rgba(22,163,74,0.11), 0 2px 8px rgba(22,163,74,0.07)'
                  e.currentTarget.style.transform = 'translateY(-3px)'
                }}
                onMouseLeave={e => {
                  e.currentTarget.style.border = '1.5px solid rgba(187,247,208,0.8)'
              ...
              >
                ...
                <div className="flex items-center gap-3 px-4 py-3.5"
                  style={{ background: 'linear-gradient(135deg, rgba(22,163,74,0.04) 0%, rgba(22,163,74,0.02) 100%)', borderBottom: '1px solid rgba(187,247,208,0.5)' }}>
                  <div className="h-9 w-9 rounded-xl flex items-center justify-center shrink-0"
                    style={{ background: 'linear-gradient(135deg,#166534,#16A34A)', boxShadow: '0 2px 8px rgba(22,163,74,0.28)' }}>
                ...
                <div className="grid grid-cols-3 divide-x divide-slate-100 border-b border-slate-100">
                  {[
                    { label: 'Models', value: ws.dataset_count, color: '#65A30D' },
    ...
    return (
      <div className="space-y-4">
        ...
        <div className="rounded-2xl overflow-hidden"
          style={{ background: 'linear-gradient(135deg, #166534 0%, #16A34A 50%, #22C55E 100%)', boxShadow: '0 4px 20px rgba(22,163,74,0.22)' }}>
            ...
            {groups.map(({ ds, reports, dist }, gi) => {
              ...
              return (
                <motion.div
                  ...
                  style={{ borderColor: 'rgba(187,247,208,0.8)', boxShadow: '0 1px 4px rgba(0,0,0,0.04)' }}
                >
                  ...
                  <button
                    ...
                    style={{ background: 'linear-gradient(135deg, rgba(22,163,74,0.04) 0%, rgba(22,163,74,0.02) 100%)' }}
                    onMouseEnter={e => { (e.currentTarget as HTMLButtonElement).style.background = 'linear-gradient(135deg, rgba(22,163,74,0.08) 0%, rgba(22,163,74,0.04) 100%)' }}
                    onMouseLeave={e => { (e.currentTarget as HTMLButtonElement).style.background = 'linear-gradient(135deg, rgba(22,163,74,0.04) 0%, rgba(22,163,74,0.02) 100%)' }}
                  >
                    <div className="h-9 w-9 rounded-xl flex items-center justify-center shrink-0"
                  ...
                  <div className="grid grid-cols-4 sm:grid-cols-6 divide-x divide-slate-100 border-t border-slate-100">
                    {[
                      ...
                      { label: 'Rels', value: ds.relationship_count, color: '#65A30D' },
          ...
          {workspaces.map((ws, wi) => {
            ...
            return (
              <motion.button
                ...
                style={{
                  border: isEmpty
                    ? '1.5px solid rgba(187,247,208,0.5)'
                    : '1.5px solid rgba(187,247,208,0.8)',
                ...
                onMouseEnter={e => {
                  e.currentTarget.style.border = isEmpty
                    ...
                    : '1.5px solid rgba(22,163,74,0.42)'
                  e.currentTarget.style.boxShadow = isEmpty
                    ? '0 4px 14px rgba(148,163,184,0.15)'
                    : '0 8px 28px rgba(22,163,74,0.11), 0 2px 8px rgba(22,163,74,0.07)'
                ...
                <div className="flex items-center gap-3 px-4 py-3.5"
                  style={{
                    background: isEmpty
                      ...
                      : 'linear-gradient(135deg, rgba(22,163,74,0.04) 0%, rgba(22,163,74,0.02) 100%)',
                  ...
                  <div className="h-9 w-9 rounded-xl flex items-center justify-center shrink-0"
                    style={{
                      ...
                      boxShadow: isEmpty ? 'none' : '0 2px 8px rgba(22,163,74,0.28)',
                    }}>
                ...
                {isEmpty ? (
                  ...
                  <>
                    ...
                    <div className="px-4 py-2.5 flex items-center justify-between">
                      ...
                      <span className="text-xs font-semibold" style={{ color: '#16A34A' }}>{wsFiltered.length} matching</span>
                    </div>
                  </>
                )}
              </motion.button>
          ...
          {selectedWs.datasets.map((ds, di) => {
            ...
            return (
              <motion.button
                ...
                style={{
                  border: isEmpty ? '1.5px solid rgba(187,247,208,0.5)' : '1.5px solid rgba(187,247,208,0.8)',
                  background: isEmpty ? 'rgba(248,250,252,0.7)' : 'white',
                  opacity: isEmpty ? 0.65 : 1,
                  boxShadow: '0 1px 4px rgba(0,0,0,0.04)',
                  cursor: 'pointer',
                ...
                onMouseEnter={e => {
                  ...
                  e.currentTarget.style.opacity = '1'
                  e.currentTarget.style.transform = 'translateY(-2px)'
                  e.currentTarget.style.boxShadow = '0 4px 14px rgba(22,163,74,0.10)'
                }}
                onMouseLeave={e => {
                  e.currentTarget.style.border = isEmpty
                    ? '1.5px solid rgba(187,247,208,0.5)'
              ...
              >
                <div className="flex items-center gap-3 px-4 py-3.5"
                  ...
                  <div className="h-9 w-9 rounded-xl flex items-center justify-center shrink-0"
                    style={{
                      background: isEmpty ? 'rgba(148,163,184,0.15)' : 'linear-gradient(135deg,#166534,#16A34A)',
                      boxShadow: isEmpty ? 'none' : '0 2px 8px rgba(22,163,74,0.25)',
                    }}>
    ...
    return (
      <div className="space-y-4">
        ...
        {modelItems.length === 0 ? (
          ...
          <div className="rounded-2xl border overflow-hidden" style={{ borderColor: 'rgba(187,247,208,0.8)', boxShadow: '0 1px 4px rgba(0,0,0,0.04)' }}>
          ...
          <div className="flex items-center gap-0.5 overflow-x-auto pb-0"
            ...
            {TABS.map(tab => (
              <button key={tab.id}
                ...
                style={{
                  position: 'relative',
                  color: activeTab === tab.id ? '#166534' : '#64748B',
                  background: activeTab === tab.id ? 'rgba(22,163,74,0.05)' : 'transparent',
